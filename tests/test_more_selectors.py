...
from plugins.more_selectors import (HollowCylinderSelector,
...
def test_HollowCylinderSelector():
    ...
    boxes = cq.Workplane().box(10,10,10).box(3,3,3).cut(innerbox).moveTo(10,0).box(2,2,10)
    vertices = boxes.vertices(HollowCylinderSelector((0,0,3), "Z",8,8,4,debug=True))
    ...
    solids = boxes.solids(HollowCylinderSelector((0,0,-6), "Z",12, 15, 2, debug=True))
...
def test_InfHollowCylinderSelector():   
    ...
    innerbox = cq.Workplane().box(5,5,10)
    boxes = cq.Workplane().box(10,10,10).box(3,3,3).cut(innerbox).moveTo(10,0).box(2,2,10)
    vertices = boxes.vertices(InfHollowCylinderSelector((0,0,3), "Z", 4, 2, debug=True))
...
def test_SphereSelector():    
    ...
    """
    sphere = cq.Workplane().sphere(5).polarArray(7, 30, 120, 2, rotate=True).box(1,2,1)
